...
class SparseTensor:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._shape = shape
        self._layout = layout
        # Optional per-point world coordinates aligned with feats/coords rows.
        # Common shape: [NNZ, 3]
        self._xyzs = xyzs
    ...
    def __cal_shape(self, feats, coords):
        shape = []
        shape.append(coords[:, 0].max().item() + 1)  # batch size
    ...
    @property
    def xyzs(self) -> Optional[torch.Tensor]:
        return self._xyzs

    @xyzs.setter
    def xyzs(self, value: Optional[torch.Tensor]):
        self._xyzs = value
    ...
    def to(self, *args, **kwargs) -> "SparseTensor":
        ...
        new_xyzs = None if self._xyzs is None else self._xyzs.to(device=device)
        return self.replace(new_feats, new_coords, xyzs=new_xyzs)
    ...
    def cpu(self) -> "SparseTensor":
        ...
        new_xyzs = None if self._xyzs is None else self._xyzs.cpu()
        return self.replace(new_feats, new_coords, xyzs=new_xyzs)
    ...
    def detach(self) -> "SparseTensor":
        ...
        new_xyzs = None if self._xyzs is None else self._xyzs.detach()
        return self.replace(new_feats, new_coords, xyzs=new_xyzs)
    ...
    def replace(
        self, feats: torch.Tensor, coords: Optional[torch.Tensor] = None, xyzs: Optional[torch.Tensor] = None
    ) -> "SparseTensor":
    ...
    def __getitem__(self, idx):
        ...
        coords = []
        feats = []
        xyzs = [] if self._xyzs is not None else None
        for new_idx, old_idx in enumerate(idx):
            coords.append(self.coords[self.layout[old_idx]].clone())
            coords[-1][:, 0] = new_idx
            feats.append(self.feats[self.layout[old_idx]])
            if xyzs is not None:
                xyzs.append(self._xyzs[self.layout[old_idx]])
        coords = torch.cat(coords, dim=0).contiguous()
        feats = torch.cat(feats, dim=0).contiguous()
        xyzs_cat = None if xyzs is None else torch.cat(xyzs, dim=0).contiguous()
        return SparseTensor(feats=feats, coords=coords, xyzs=xyzs_cat)
...
def sparse_cat(inputs: List[SparseTensor], dim: int = 0) -> SparseTensor:
    ...
    if dim == 0:
        ...
        xyzs = (
            None
            if any(inp.xyzs is None for inp in inputs)
            else torch.cat([inp.xyzs for inp in inputs], dim=0)
        )
        output = SparseTensor(coords=coords, feats=feats, xyzs=xyzs)
    else:
